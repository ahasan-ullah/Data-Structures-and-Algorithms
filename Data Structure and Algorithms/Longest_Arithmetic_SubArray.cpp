// Problem--
// An arithmetic array is an array that contains at least two integers and the differences between consecutive integers are equal. For example,[9, 10], [3, 3, 3],and [9, 7, 5, 3] are arithmetic arrays, while [1, 3, 3,7], [2, 1, 2], and [1, 2, 4] arenot arithmetic arrays.
...
int main(){
  ...
  while(t--){
    ...
    int pd=arr[1]-arr[0],current=2,j=2,ans=2;
    while(j<n){
      if(pd==(arr[j]-arr[j-1])){
        current++;
      }
